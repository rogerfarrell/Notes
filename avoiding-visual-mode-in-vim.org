...
your back.[fn:2] One occasion where visual mode is absolutely the
correct tool is when running a shell command on a selection within a
file. (E.g.
[[file:running-pandoc-on-a-selection-within-a-file-in-vim.org]].)

[fn:1] Chris Toomey gave a great talk on this approach to Vim:
       [[file:references/mastering-the-vim-language.org]]

[fn:2] Though, you might try using using marks in Vim before resorting
       to the adjustable wrench: [[file:references/using-marks-in-vim.org]]
